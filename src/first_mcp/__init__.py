"""
First MCP Server - A comprehensive MCP server package.

Provides MCP tools for:
- Memory management with TinyDB backend and semantic search
- Text embedding and similarity scoring (Google AI)
- Workspace file I/O management (including anchor-based text editing)
- Biblical text lookup (ESV, auto-downloaded) via reference strings
- Weather services (OpenWeatherMap + Yr.no)
- Mathematical and time-difference calculators
- Calendar and date utilities

GitHub: https://github.com/TobiSan5/first-mcp
Author: Torbjørn Wikestad <torbjorn.wikestad@gmail.com>
"""

__version__ = "1.3.0"
__author__ = "Torbjørn Wikestad"
__email__ = "torbjorn.wikestad@gmail.com"

VERSION = (1, 3, 0)

PACKAGE_NAME = "first-mcp"
DESCRIPTION = "A comprehensive MCP (Model Context Protocol) server with memory, workspace, weather, and utility tools"
HOMEPAGE = "https://github.com/TobiSan5/first-mcp"
